    ...
    <ps-modal ref="psmodal" maxWidth="868px" line="hidden" :isClickOut='false' theme="rounded-lg shadow-xl overflow-hidden dark:bg-feSecondary-800" class=' z-20'>
        <template #title>
            <div class="flex justify-between shadow p-6 dark:bg-feSecondary-800">
                <ps-label textColor="text-2xl font-semibold text-feSecondary-800 dark:text-feSecondary-50"> {{ $t('item_detail__safety_tips') }} </ps-label>
        ...
        <template #body>
                <div class="h-52 overflow-y-auto my-6 ps-6 pe-10">
                    <ps-label class="text-base font-normal text-feSecondary-700 dark:text-feSecondary-400"> {{ aboutUsProvider.aboutus?.data?.safetyTips }} </ps-label>
                </div>
        </template>
        <template #footer>
