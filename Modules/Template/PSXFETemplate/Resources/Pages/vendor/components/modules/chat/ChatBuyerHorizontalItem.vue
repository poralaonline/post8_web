<template>
    <div class="cursor-pointer " @click="gotoDetail(chathistory)">
        ...
        <!-- Pscard -->
        <div :class="`h-[80px] sm:h-[100px] rounded-lg shadow-sm overflow-hidden ${chathistory?.sellerUnreadCount > 0 ? 'bg-feAchromatic-100  dark:bg-feSecondary-700' : ''} dark:bg-feArchromatic-900`">
           <div class="flex h-full ">
                <div class="p-2 h-full  sm:w-[100px] relative flex items-center">
                ...
                <div class="grow px-3 py-3">
                    <ps-label textColor="font-medium text-fePrimary-600 text-xs sm:text-lg"> {{chathistory ? chathistory.buyer.userName : ''}} </ps-label>
                    <div class="flex justify-between items-center w-full">
                        ...
                        <ps-label class="font-normal text-xs sm:text-base lg:block hidden" v-if="chathistory">{{ chathistory.item.title }}</ps-label>
