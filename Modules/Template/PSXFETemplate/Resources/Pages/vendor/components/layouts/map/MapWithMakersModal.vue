<template>
    <ps-modal ref="psmodal" maxWidth="370px" :isClickOut='false' line="hidden" theme= "bg-feAchromatic-50 dark:bg-feSecondary-800 lg:px-6 px-4 py-6 rounded-x " class=' z-20'>
        ...
        <template #body>
            <div class="flex">
                <div v-if="!isGoogleMap" class="w-full h-68 flex justify-center items-center">
                ...
                <div v-else class="w-full h-68 flex justify-center items-center">
                    <google-map-with-makers class="w-full h-68" v-if="mapReady" @onChange="onPointerMove" :dir="$page.props.dir" :lat="lat" :lng="lng" :radius="getMiles()" :mapKey="googleMapKey"/>
                    <ps-label v-else class="flex-grow-0"> {{ $t("fe_map_with_marker_moadl__loading") }} </ps-label>
                </div>
            </div>
...
import GoogleMapWithMakers from '@template1/vendor/components/layouts/map/GoogleMapWithMakers.vue';
...
export default defineComponent({
    ...
    setup() {
        ...
        async function openModal(latStr, lngStr,mile, clickFunction: Function) {
            isGoogleMap.value = appInfoStore.appInfo.data.frontendConfigSetting.googleMap == 1 ? true : false;
            googleMapKey.value = appInfoStore.appInfo.data.frontendConfigSetting.mapKey;
