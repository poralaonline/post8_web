        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{$t('core_vendor__payment_status_info')}}</ps-label-header-6>
                </div>
                <!-- card header end -->

                <!-- card body start -->
                <div class="px-4 pt-6 dark:bg-backgroundDark">
                    <form @submit.prevent="handleSubmit(paymentStatus.vendor_id, paymentStatus.id)">
                    <div class="grid w-full sm:w-1/2 gap-6">
                        <div>
                            <ps-label class="text-base">{{ $t('core_vendor__payment_status_name') }}<span class="text-red-500 ms-1">*</span></ps-label>
                            <ps-input ref="name" type="text" v-model:value="form.name" :placeholder="$t('core_vendor__payment_status_title')"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        let form = useForm({
            name: props.paymentStatus.name,
            description: props.paymentStatus.description,
            status: props.paymentStatus.status == 1 ? true : false,
            "_method": "put"
        });

        function handleSubmit(vendor_id, id) {
            this.$inertia.post(route("vendor_payment_status.update", [vendor_id, id]), form, {
                forceFormData: true,
                onBefore: () => {
                    loading.value = true;
                },
        ...
        breadcrumb() {
            ...
            return [
                ...
                },
                {
                    ...
                    url: route('vendor_payment_status.index'),
                },
                {
                    label: trans('vendor_panel__edit_payment_status'),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route('vendor_payment_status.index'));
        },
    },
})
</script>
