        ...
        @handleSort="handleSorting"  @handleSearch="handleSearching" @handleRow="handleRow"
            ...
            <template #tableRow="rowProps">
                <ps-toggle v-show="colFilterOptions.filter((item) => item.key == 'status').length == 0
                            ? true : !colFilterOptions.filter((item) => item.key == 'status')[0].hidden " :disabled="rowProps.row.authorization.update ? false : true" v-if="rowProps.field == 'status'" :selectedValue="rowProps.row.status == 1 ? true : false" @click="handlePublish(rowProps.row.id,rowProps.row.authorization.update)"></ps-toggle>
            </template>
