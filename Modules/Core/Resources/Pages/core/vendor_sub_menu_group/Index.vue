<template>
    <Head :title="$t('vendor_sub_menu_group_module')" />
    <ps-layout>
        <div class="">
            <!-- breadcrumb start -->
           ...
           <!-- alert banner start -->
            ...
            <ps-table2 :row="row" :search="search" :object="sub_menu_groups" :colFilterOptions="colFilterOptions"
                ...
                @handleRow="handleRow" :searchable="showFilter">
                    ...
                    <template #button>
                        <ps-button v-if="can.createCoreSubMenu" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" />{{$t('add_vendor_sub_menu_group')}}</ps-button>
                    </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function confirmClicked(id) {
            ...
            ps_danger_dialog.value.openModal(
                ...
                () => {
                        this.$inertia.delete(route('vendor_sub_menu_group.destroy',id),{
                            onSuccess: () => {
                            visible.value = true;
                            setTimeout(() => {
                                visible.value = false;
        ...
        function handleEdit(id){
            this.$inertia.get(route('vendor_sub_menu_group.edit',id));
        }
        function handlePublish(id,hasPermission) {
            if(hasPermission){
                this.$inertia.put(route('vendor_sub_menu_group.statusChange', id));
            }
        }
        function handleCreate() {
            this.$inertia.get(route("vendor_sub_menu_group.create"));
        }
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('vendor_sub_menu_group.index'),
                {
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('vendor_sub_menu_group_module'),
    ...
    methods: {
        FilterOptionshandle(value) {
            ...
            router.post(route('vendor_sub_menu_group.screenDisplayUiSetting.store'),
                {
                    value,
