            ...
            <template #tableRow="rowProps">
                ...
                <div class="flex flex-row" v-if="rowProps.field == 'accept'">
                    <ps-button :disabled="!rowProps.row.authorizations.update" @click="[form.status='accept',handleStatusChange(rowProps.row.id)]"
                        ...
                        focus="focus:outline-none focus:ring focus:ring-green-200">{{ $t('core__be_apply_lbl') }}</ps-button>
                </div>
...
import PsDangerDialog from "@/Components/Core/Dialog/PsDangerDialog.vue";
    ...
    export default defineComponent({
        ...
        setup(props) {
            ...
            function confirmDeleteClicked(id) {
                ps_danger_dialog.value.openModal(
                    ...
                    () => {
                        router.delete(route("reject_vendor.destroy", id), {
                            onSuccess: () => {
                                visible.value = true;
                                setTimeout(() => {
                                    visible.value = false;
            ...
            function handleStatusChange(id) {
                this.$inertia.put(route('reject_vendor.statusChange', id), form);
            }
            ...
            function handleSearchingSorting(page = null, row = null) {
                router.get(route('reject_vendor.index'),
                    {
        ...
        methods: {
            handleDetail(id) {
                this.$inertia.get(route('reject_vendor.show', id));
            },
            handleDownload(id) {
                this.$inertia.get(route('reject_vendor.download_document', id));
            }
        },
        created() {
