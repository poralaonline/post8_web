<template>
    <Head :title="$t('core_add_vendor_menu_group_module')" />
    <ps-layout>
        <div class="">
            <!-- breadcrumb start -->
           ...
           <div class="max-w-lg">
                <div class="mt-8 rounded-lg bg-white dark:bg-secondaryDark-black  shadow-sm">
                    <ps-label class="text-lg px-4 py-2.5 bg-primary-50 dark:bg-primary-900">{{$t('core_add_vendor_menu_group_module')}}</ps-label>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit() {
            this.$inertia.post(route('vendor_menu_group.store'), form, {
                forceFormData: true,
        ...
        breadcrumb() {
            return [
                ...
                },
                {
                    ...
                    url: route('vendor_menu_group.index'),
                },
                {
                    label: trans('core_add_vendor_menu_group_module'),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route('vendor_menu_group.index'));
        },
    },
})
</script>
