<template>
    <Head :title="$t('vendor_menu_group_module')" />
    <ps-layout>
        <div class="">
            <!-- breadcrumb start -->
        ...
        class="text-white mb-5 sm:mb-6 lg:mb-8"
         iconColor="white">{{status.msg}}</ps-banner-icon>
            ...
            <ps-table2 :row="row" :search="search" :object="menu_groups" :colFilterOptions="colFilterOptions"
                ...
                <template #button>
                   <ps-button v-if="can.createCoreMenu" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" />{{$t('core_add_vendor_menu_group_module')}}</ps-button>
                </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function confirmClicked(id) {
            ...
            ps_danger_dialog.value.openModal(
                ...
                () => {
                        this.$inertia.delete(route('vendor_menu_group.destroy',id),{
                            onSuccess: () => {
                            visible.value = true;
                            setTimeout(() => {
                                visible.value = false;
        ...
        function handleEdit(id){
            this.$inertia.get(route('vendor_menu_group.edit',id));
        }
        function handlePublish(id,hasPermission) {
            if(hasPermission){
                this.$inertia.put(route('vendor_menu_group.statusChange', id));
            }
            
        }
        function handleCreate() {
            this.$inertia.get(route("vendor_menu_group.create"));
        }
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('vendor_menu_group.index'),
                {
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('vendor_menu_group_module'),
    ...
    methods: {
        FilterOptionshandle(value) {
            ...
            router.post(route('vendor_menu_group.screenDisplayUiSetting.store'),
                {
                    value,
