<template>
    <Head :title="$t('edit_vendor_menu_group')" />
    <ps-layout>
        <div class="">
            <!-- breadcrumb start -->
           ...
           <div class="max-w-lg">
                <div class="mt-8 rounded-lg bg-white dark:bg-secondaryDark-black  shadow-sm">
                    <ps-label class="text-lg px-4 py-2.5 bg-primary-50 dark:bg-primary-900">{{$t('edit_vendor_menu_group')}}</ps-label>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit(id) {
            this.$inertia.post(route('vendor_menu_group.update', id), form, {
                forceFormData: true,
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('edit_vendor_menu_group'),
