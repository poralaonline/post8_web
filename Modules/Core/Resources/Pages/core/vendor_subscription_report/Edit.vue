<template>
    ...
    <Head :title="$t('vendor_subscription_report_detail')" />
<ps-layout>
    ...
    <ps-card class="w-full h-auto">
        <div class="rounded-lg ">
            ...
            <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4">
                <ps-label-header6>{{$t('vendor_subscription_report_detail')}}</ps-label-header6>
            </div>
            <!-- card header end -->
            ...
            <div class="px-4 pt-6 dark:bg-backgroundDark">
                <form @submit.prevent="handleSubmit(transaction.id)" >
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <ps-label-header6>{{$t('vendor_subscription_plan_info')}}</ps-label-header6>
                            <div class="grid grid-cols-2 gap-3 mt-6">
                                <ps-label textColor="text-secondary-400">{{$t('core_be__plan_title')}}</ps-label>
                                <ps-label> : {{transaction.package.payment_info.value}}</ps-label>

                                <ps-label textColor="text-secondary-400">{{$t('core_be__plan_duration')}}</ps-label>
                                <ps-label> : {{transaction.package.payment_attributes.duration}}</ps-label>

                                <ps-label textColor="text-secondary-400">{{$t('core_be__plan_sale_price')}}</ps-label>
                                <ps-label> : {{ transaction.package.payment_attributes.currency_symbol }} {{transaction.package.payment_attributes.sale_price}}</ps-label>

                                <ps-label textColor="text-secondary-400">{{$t('core_be__plan_discount_price')}}</ps-label>
                                <ps-label> : {{transaction.package.payment_attributes?.discount_price}}</ps-label>
                            </div>
                        </div>
                        <div>
                            <ps-label-header6>{{$t('package_transition_info')}}</ps-label-header6>
                            <div class="grid grid-cols-2 gap-3 mt-6">
                                <ps-label textColor="text-secondary-400">{{$t('core_be__transaction_id')}}</ps-label>
                                <ps-label> : {{transaction.transaction_id}}</ps-label>
                                ...
                                <ps-label textColor="text-secondary-400">{{$t('status')}}</ps-label>
                                <ps-label> : {{transaction.status == 1 ? $t('core__be_active_status') : $t('core__be_expired_status')}}</ps-label>

                                <ps-label textColor="text-secondary-400">{{$t('core_be__purchased_date')}}</ps-label>
                                <ps-label> : {{moment(transaction.added_date).format($page.props.dateFormat)}}</ps-label>

                                <ps-label textColor="text-secondary-400">{{$t('core_be__expired_date')}}</ps-label>
                                <ps-label> : {{moment(transaction.expired_date).format($page.props.dateFormat)}}</ps-label>
                            </div>
                        </div>
                    </div>
...
export default defineComponent({
    ...
    setup(props) {
        console.log(props.transaction, props.packages);
        ...
        let post_count_filter = ref(props.transaction.user.userRelation.filter((field) => field.core_keys_id === 'ps-usr00004'));
    ...
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans('vendor_subscription_report_module'),
                    url: route('vendor_subscription_report.index'),
                },
                {
                    label: trans('vendor_subscription_report_detail'),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route('vendor_subscription_report.index'));
        },
    },
})
</script>
