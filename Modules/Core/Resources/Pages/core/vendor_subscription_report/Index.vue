                ...
                <template #searchRight>
                    ...
                    <template #Filter>
                        ...
                        <ps-dropdown align="" class=" h-10">
                            <template #select>
                                <ps-dropdown-select :placeholder="$t('core_be__plan')"
                 ...
                 <template #tableRow="rowProps">
                    ...
                    <div class="flex flex-row mb-2" v-if="rowProps.field == 'duration'">
                        <ps-label>
                            {{ duration.filter((duration)=>duration.id == rowProps.row.package.payment_attributes.duration)[0].name }}
                        </ps-label>
                    </div>
                    <div class="" v-if="rowProps.field == 'status'">
                        <ps-label v-if="rowProps.row.status == 1" class="flex flex-row" textColor="text-green-500">
                            <ps-label class="w-2 h-2 my-auto rounded-full me-2" textColor="bg-green-500"></ps-label>
                            {{ $t('core__be_active_status') }}
                        </ps-label>
                        <ps-label v-if="rowProps.row.status == 0" class="flex flex-row" textColor="text-red-500">
                            <ps-label class="w-2 h-2 my-auto rounded-full me-2" textColor="bg-red-500"></ps-label>
                            {{ $t('core__be_expired_status') }}
                        </ps-label>
                    </div>
                </template>
            </ps-table2>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const columns = [
            ...
            {
                label: trans("core_be__transaction_id"),
                field: "transaction_id",
                type: "String",
                action: "Action",
            },
            {
                label:trans("core_be__plan"),
                field: "package_id",
                relation: "package",
                type: "String",
            ...
            {
                label: trans("payment__be_vendor_sp_duration"),
                field: "duration",
                relation: "package",
                type: "String",
                relationField: "payment_attributes",
                action: "Action",
                sort:false
            },
            {
            ...
            {
                label: trans("core_be__purchased_date"),
                field: "added_date",
                type: "Timestamp",
                action: "Action",
            },
            {
                label: trans("core_be__expired_date"),
                field: "expired_date",
                type: "Timestamp",
                action: "Action",
            },
        ...
        function handleDetail(id){
            this.$inertia.get(route('vendor_subscription_report.show',id));
        }
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('vendor_subscription_report.index'),
                {
