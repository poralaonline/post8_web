<template>
    <Head :title="$t('edit_vendor_module')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{$t('core__vendor_module_info')}}</ps-label-header-6>
                </div>
                <!-- card header end -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit() {
            this.$inertia.post(route('vendor_module_registering.update', props.module.id), form, {
                forceFormData: true,
        ...
        breadcrumb() {
            ...
            return [
                ...
                },
                {
                    ...
                    url: route('vendor_module_registering.index'),
                },
                {
                    label: trans('edit_vendor_module'),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route('vendor_module_registering.index'));
        },
    }
})
</script>
